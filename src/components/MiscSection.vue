<script setup lang="ts">
import { useGameDataStore } from '../stores/gameData';
import { EVENT_FLAGS, MAX_COINS } from '../codec';
import CollapsibleSection from './CollapsibleSection.vue';

const store = useGameDataStore();

function clampCoins(value: string) {
  let n = parseInt(value, 10);
  if (isNaN(n) || n < 0) n = 0;
  if (n > MAX_COINS) n = MAX_COINS;
  store.setCoins(n);
}
</script>

<template>
  <CollapsibleSection title="Miscellaneous">
    <template #header-right>
      <label class="flex items-center gap-2 text-sm font-normal text-gray-300">
        Coins
        <input
          type="number"
          min="0"
          :max="MAX_COINS"
          :value="store.gameData.coins"
          @change="clampCoins(($event.target as HTMLInputElement).value)"
          class="w-28 border border-gray-700 bg-gray-800 text-amber-50 rounded px-2 py-0.5 text-sm text-right tabular-nums"
        >
      </label>
    </template>
    <div class="grid grid-cols-3 gap-x-4 gap-y-1">
      <label
        v-for="(flag, index) in EVENT_FLAGS"
        :key="index"
        class="flex items-center gap-1.5 text-sm text-gray-300 cursor-pointer"
        :title="flag.description"
      >
        <input
          type="checkbox"
          :checked="store.isFlagSet(index)"
          @change="store.setFlag(index, ($event.target as HTMLInputElement).checked)"
          class="accent-amber-600"
        >
        {{ flag.name }}
        <span v-if="flag.name === 'Hsu Died'" class="text-[10px] text-gray-500 cursor-help" title="Inverted: unchecked = Hsu rescued (quest complete)">?</span>
      </label>
    </div>
  </CollapsibleSection>
</template>
